...
func NewPodAutoscalerProfileInternal(profile *datadoghq.DatadogPodAutoscalerProfile) (PodAutoscalerProfileInternal, error) {
...
func (p *PodAutoscalerProfileInternal) UpdateFromProfile(profile *datadoghq.DatadogPodAutoscalerProfile) error {
...
func (p *PodAutoscalerProfileInternal) UpdateFromStatus(status *datadoghq.DatadogPodAutoscalerProfileStatus) {
	for _, cond := range status.Conditions {
		switch {
		case cond.Type == string(datadoghq.DatadogPodAutoscalerProfileValidCondition) && cond.Status == metav1.ConditionFalse:
			p.valid = false
...
// BuildStatus constructs a DatadogPodAutoscalerProfileStatus from the internal state.
