...
func validTestProfile(name string) model.PodAutoscalerProfileInternal {
	pi, _ := model.NewPodAutoscalerProfileInternal(&datadoghq.DatadogPodAutoscalerProfile{
...
func TestWorkloadWatcherScanWorkloads(t *testing.T) {
	...
	t.Run("Labelled Deployment with existing profile", func(t *testing.T) {
		...
		w.scanWorkloads(gvkr, newFakeLister(deployment), refs)
	...
	t.Run("Multiple workloads one profile", func(t *testing.T) {
		...
		refs := make(map[string][]model.NamespacedObjectReference)
		w.scanWorkloads(gvkr, newFakeLister(dep1, dep2), refs)
...
func TestWorkloadWatcherReconcile(t *testing.T) {
	...
	t.Run("Sets workload refs on profile", func(t *testing.T) {
		...
		w.informers = []workloadInformer{
			{gvkr: gvkr, lister: newFakeLister(deployment)},
		}
	...
	t.Run("Clears workload refs when label removed", func(t *testing.T) {
		...
		w.informers = []workloadInformer{
			{gvkr: gvkr, lister: newFakeLister()},
		}
...
// fakeLister implements cache.GenericLister for testing.
type fakeLister struct {
	objects []runtime.Object
}

func newFakeLister(objs ...*unstructured.Unstructured) *fakeLister {
	objects := make([]runtime.Object, len(objs))
...
func (f *fakeLister) ByNamespace(_ string) cache.GenericNamespaceLister {
	return nil
}
