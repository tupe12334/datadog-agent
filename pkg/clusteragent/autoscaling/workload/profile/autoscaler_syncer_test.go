...
func TestAutoscalerSyncerUpdateDPAOnTemplateChange(t *testing.T) {
	...
	updatedProf, _ := model.NewPodAutoscalerProfileInternal(&datadoghq.DatadogPodAutoscalerProfile{
...
func TestAutoscalerSyncerInvalidProfileMarksExistingDPAsDeleted(t *testing.T) {
	...
	invalidProf, _ := model.NewPodAutoscalerProfileInternal(&datadoghq.DatadogPodAutoscalerProfile{
		ObjectMeta: metav1.ObjectMeta{Name: "high-cpu", Generation: 2},
...
func TestDpaNameFromKey(t *testing.T) {
	assert.Equal(t, "web-app-abc123", dpaNameFromKey("prod/web-app-abc123"))
	assert.Equal(t, "name", dpaNameFromKey("ns/name"))
	assert.Equal(t, "justname", dpaNameFromKey("justname"))
}
